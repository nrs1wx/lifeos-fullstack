...
- Зарегистрировать новый аккаунт (кнопка "Register") — email/пароль сохранятся
  в базе данных, JWT-сессия выдаётся сразу после успешной регистрации.
- Войти под уже созданным аккаунтом по email и паролю.
...
- **Авторизация**: регистрация и вход по email/паролю, пароли хранятся
  захешированными (bcrypt), сессия — JWT-токен на 30 дней.
...
2. На [Render](https://render.com) нажмите **New → Blueprint**, подключите
...
3. В разделе **Environment** добавьте: `DATABASE_URL` (строка Neon),
   `JWT_SECRET` (результат команды выше), `ANTHROPIC_API_KEY` (ключ из
   console.anthropic.com, можно оставить пустым), `ANTHROPIC_MODEL`, `PORT=8787`
   и пока `CORS_ORIGIN=http://localhost:3000`. Нажмите **Save
   Changes**, дождитесь deploy. Откройте `https://ВАШ-СЕРВИС.onrender.com/api/health`:
   ответ должен быть `{ "status": "ok" }`.
4. На [Vercel](https://vercel.com) выберите **Add New → Project**, импортируйте
...
5. Скопируйте выданный Vercel домен. Вернитесь в Render → Environment и замените
